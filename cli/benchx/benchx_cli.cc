...
#include "shell/runtime/bts/lynx_bts_runtime_proxy_impl.h"
#include "shell/runtime/common/module_delegate_impl.h"
...
int main(int argc, char** argv) {
  ...
  run_command.add_argument("--wait")
      .help(
          "Wait for milliseconds before exiting. Note: if --wait-for-id is "
          "also specified, the waiting time starts after the id is found.")
      .scan<'i', int32_t>();
  ...
  if (program.is_subcommand_used(run_command)) {
    ...
    auto run = [&]() {
      ...
      auto module_manager =
          std::make_shared<lynx::runtime::js::LynxModuleManager>();
      ...
      auto on_runtime_actor_created = [&](auto& actor) {
        ...
        runtime_proxy =
            std::make_shared<lynx::shell::LynxBTSRuntimeProxyImpl>(actor);
        module_manager->runtime_proxy = runtime_proxy;
      };

      auto runtime_flags =
          lynx::shell::CalcRuntimeFlags(false, true, false, false);
    ...
    std::thread([&]() {
      for (int i = 0; i < max_runs; ++i) {
        ...
        if (wait_time.has_value() && *wait_time > 0) {
